 ...
 *   3. Rate-limits submissions per authenticated user.
 *   4. Passes the upload through File_Validator (4-layer check).
...
class Submission_Handler {
	...
	public function enqueue_assets(): void {
		...
		wp_localize_script(
			...
			[
				...
				'maxSizeMB'  => round( File_Validator::MAX_BYTES / 1048576, 1 ),
				'i18n'       => [
					'uploading'   => __( 'Uploading and scanning your file…', 'urban-canvas' ),
					'success'     => __( 'Your submission has been received! We\'ll be in touch soon.', 'urban-canvas' ),
	...
	public function render_form( array $atts ): string {
		...
		ob_start();
		load_template( UC_PLUGIN_DIR . 'templates/submission-form.php', false, $atts );
		return ob_get_clean();
	}
	...
	public function handle_submission(): void {
		...
		// ── Rate limiting ─────────────────────────────────────────────────────
		$rate_key    = 'uc_rate_' . $user->ID . '_' . gmdate( 'Ymd' );
		...
		$artwork_title = sanitize_text_field( wp_unslash( $_POST['artwork_title']  ?? '' ) );
		$artwork_desc  = sanitize_textarea_field( wp_unslash( $_POST['artwork_desc'] ?? '' ) );

		if ( empty( $artist_name ) || empty( $artwork_title ) ) {
			wp_send_json_error( [ 'message' => __( 'Artist name and artwork title are required.', 'urban-canvas' ) ], 400 );
		}

		// Length caps.
		if ( mb_strlen( $artist_name ) > 100 || mb_strlen( $artwork_title ) > 200 ) {
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized — sanitized inside validate()
		...
		if ( ! move_uploaded_file( $upload_data['tmp_path'], $dest_path ) ) {
			...
			wp_send_json_error( [ 'message' => __( 'File storage failed. Please contact support.', 'urban-canvas' ) ], 500 );
		}
		...
		wp_send_json_success(
			[
				'message'       => __( 'Your submission has been received! We\'ll be in touch soon.', 'urban-canvas' ),
	 ...
	 */
	private function get_private_dir(): string {
		$dir = WP_CONTENT_DIR . '/' . self::PRIVATE_SUBDIR;

		if ( ! is_dir( $dir ) ) {
			wp_mkdir_p( $dir );

			// Place a hard .htaccess block in case the server ever maps this path.
			$htaccess = $dir . '/.htaccess';
			if ( ! file_exists( $htaccess ) ) {
				file_put_contents( // phpcs:ignore
					$htaccess,
					"# Deny all direct web access to this directory.\n" .
