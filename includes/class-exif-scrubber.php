 ...
 *
 * Automatically strips ALL EXIF, IPTC, GPS, and XMP metadata from every
 * image uploaded through the Urban Canvas portal, protecting the privacy
 * and physical locations of youth participants.
 *
 * Strategy:
 *   - JPEG/PNG: re-encode through GD (lossy-free at quality 100) which
 *     discards all metadata by design — GD never copies EXIF to output.
 *   - PDF: metadata scrubbing via string-level stripping is unreliable;
 *     the plugin logs a warning and stores PDFs in a non-web-accessible
 *     directory under /private/ to prevent direct hotlinking.
 *
 * Hooks into our submission handler (not wp_handle_upload) to ensure
 * scrubbing occurs BEFORE the file is moved to its final location.
 *
 * @package UrbanCanvas
 */
...
class Exif_Scrubber {
	...
	public function init(): void {
		// Hook priority 5 — runs before Submission_Handler moves the file.
		add_filter( 'uc_before_store_upload', [ $this, 'scrub' ], 5 );
	}

	/**
	 * Strip metadata from the temporary upload path.
